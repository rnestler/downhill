...
\documentclass[
	  ...
	  listof=totocnumbered,							%Tab. und Abb.-verzeichnis nummeriert	
		final,												%für endversion (draft -> ohne bilder und links, 
		                              %anzeige von Fehlern)
		parskip=half,									%Halbe Zeile Abstand zwischen Absätzen.
		twoside=false									%zweiseitig
		]{scrreprt} 	

\usepackage{scrhack}					%beseitigt warnung wegen float+koma inkompatibilität
...
\usepackage{babelbib}					%für deutsches literaturverzeichnis
%\usepackage{scrpage2}					%pagestyle
\usepackage{lastpage}					%für Referenzen auf letzte Seite
\usepackage{textcomp}					%einige Sonderzeichen
\usepackage{graphicx}	        %grafiken in jpeg,png
\usepackage{epstopdf}		    	%für .eps vektorgrafiken
\usepackage{color}						%benötigt für farbeinstellungen
...
\usepackage{paralist} 				%kompakte Aufzählung: \begin{compactitem}
\usepackage{float}						%für H option bei floats(figure,table)
\usepackage{lmodern}					%moderne Schrift
\usepackage{url}				   	 	%korrekte darstellung von urls (mit verlinkung)
\usepackage{changepage}       %für adjustwidth auf titelseite
\usepackage{fancyhdr}					%für \pagestyle{fancy} -> kopf/fusszeile über Textkörper hinaus
\usepackage{acronym}					%abkürzungsverzeichnis, Anwendung: \ac{KDE}
\usepackage{cellspace}				%für S-operator in Tabellen -> Formeln berühren Ränder nicht
...
%-------------------------------------------------	
\definecolor{commentColor}{rgb}{0.3,0.3,0.3}	%Grau für kommentare in Listings
\usepackage{listings}
\lstset{ %
...
%-------------------------------------------------
% Abkürzungsverzeichnis implementieren
%-------------------------------------------------									
\usepackage{nomencl} 				
...
%marginalien linksbündig mit \marg{text der marginalie}
...
\fancyhfoffset[LE,RO]{35mm} 			%header und footer nach links verlÃƒÂ¤ngern (ÃƒÂ¼ber marginalien)
...
\renewcommand{\section}{\FloatBarrier \origsection}		%\section überschreiben 
